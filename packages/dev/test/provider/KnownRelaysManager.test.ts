...
//
  describe('DefaultRelayScore', function () {
    it.only('should subtract penalty from a relay for each known failure', async function () {
      const a = {
        relayUrl: 'a50',
        baseRelayFee: toBN('0'),
        pctRelayFee: toBN('50')
      ...
      const relayScoreLowFees = await DefaultRelayScore([a, b, c, d, e], toBN('1000000'), toBN('1000000'))

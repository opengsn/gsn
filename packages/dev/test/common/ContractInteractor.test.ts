...
import { LoggerConfiguration } from '@opengsn/provider'
import { RelayMetadata } from '@opengsn/common/dist/types/RelayTransactionRequest'
...
contract.only('ContractInteractor', function (accounts) {
  ...
  context('#validateRelayCall', () => {
    ...
    it('should return relayCall revert reason', async () => {
      ...
      const localViewCallParameters = await relayClient._getLocalViewCallParameters({ relayRequest, metadata })
      const ret = await contractInteractor.validateRelayCall(localViewCallParameters)
  ...
  describe('#splitRange', () => {
    ...
    it('split with exactly 1 block for last range', () => {
      ...
      assert.deepEqual(splitRange[20], {
        fromBlock: 200,
        toBlock: 200
      })
    })
  ...
  context('#_getPastEventsPaginated', function () {
    ...
    it('should split requested events window into necessary number of parts', async function () {
      ...
      await contractInteractor.getPastEventsForHub([], {
        fromBlock: 100,
        toBlock: 200
      })
    ...
    context('with stub 100 blocks getLogs limit', function () {
      before(function () {
        ...
        sinon.stub(contractInteractor, '_getPastEvents').callsFake(async function (contract: any, names: EventName[], extraTopics: Array<string[] | undefined>, options: PastEventOptions): Promise<any> {
      ...
      it('should break large request into multiple chunks', async () => {
        const ret = await contractInteractor.getPastEventsForHub([], {
          fromBlock: 1,
          toBlock: 300
        })

...
import {
  ...
  PenalizerInstance,
  RelayHubInstance, RelayRegistrarContract, RelayRegistrarInstance,
...
contract('ContractInteractor', function (accounts) {
  ...
  context('#LightTruffleContract', () => {
    ...
    before(async () => {
      ...
      await contractInteractor.init()
      relayReg = await RelayRegistrar.new(accounts[0])
      lightreg = await contractInteractor._createRelayRegistrar(relayReg.address)

      await relayReg.registerRelayer(AddressZero, {
        blockNumber: 10, baseRelayFee: 11, pctRelayFee: 12, relayManager: accounts[1], url: 'url'
      })
      await relayReg.registerRelayer(AddressZero, {
        blockNumber: 20, baseRelayFee: 21, pctRelayFee: 22, relayManager: accounts[2], url: 'url2'
      })
    ...
    it('should get matching numeric return value', async () => {
      expect(await lightreg.countRelays())
        .to.deep.eq(await relayReg.countRelays())
    });

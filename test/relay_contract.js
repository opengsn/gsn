...
contract("RelayHub", function (accounts) {
	...
	it("test_perform_relay_send_message", async function () {
		let result = await rhub.relay(from, to, transaction, transaction_fee, gas_price, gas_limit, relay_nonce, sig, { gasPrice: gas_price, gasLimit: 100000000 });
		relay_nonce++;
		...
		var logs_messages = await waitAllContractEventGet(sr.contract.SampleRecipientEmitted({}, { fromBlock: 0, toBlock: 'latest' }));
	...
	it("should not accept relay requests from unknown addresses", async function () {
		...
		sig = await getTransactionSignature(accounts[0], digest)
		try {
			await rhub.relay(from, to, transaction, transaction_fee, gas_price, gas_limit, relay_nonce, sig, { from: accounts[6], gasPrice: gas_price, gasLimit: 100000000 });
			assert.fail();
		}
		catch (error) {
	...
	it("should not accept relay requests with gas price lower then user specified", async function () {
		try {
			await rhub.relay(from, to, transaction, transaction_fee, gas_price, gas_limit, relay_nonce, sig, { gasPrice: gas_price - 1, gasLimit: 100000000 });
			assert.fail();
		}
		catch (error) {
	...
	it("should not accept relay requests if destination recepient doesn't approve it", async function () {
		...
		try {
			await rhub.relay(from, to, transaction, transaction_fee, gas_price, gas_limit, relay_nonce, sig, { gasPrice: gas_price, gasLimit: 100000000 });
			assert.fail();
		}
		catch (error) {
	...
	it("should not accept relay requests if gas limit is too low for a relayed transaction", async function () {
		...
		try {
			await rhub.relay(from, to, transaction, transaction_fee, gas_price, gas_limit, relay_nonce, sig, { gasPrice: gas_price, gas: gas_limit + gas_reserve });
			assert.fail();
		}
		catch (error) {
	...
	it("should penalize relay for signing two distinct transactions with the same nonce", async function () {
		...
		let res = await rhub.penalize_repeated_nonce(unsignedTransaction1Encoded, sig1, unsignedTransaction2Encoded, sig2, { from: snitching_account, gasPrice: gasPricePenalize, gasLimit: 100000000 });
	...
	it("should revert an attempt to penalize relay with two identical transactions", async function () {
		...
		try {
			await rhub.penalize_repeated_nonce(unsignedTransaction1Encoded, sig1, unsignedTransaction1Encoded, sig1, { from: snitching_account, gasPrice: gasPricePenalize, gasLimit: 100000000 });
			assert.fail()
		}
		catch (error) {
	...
	it("should revert an attempt to penalize relay with two transactions with different nonce", async function () {
		...
		try {
			await rhub.penalize_repeated_nonce(unsignedTransaction1Encoded, sig1, unsignedTransaction2Encoded_nextNonce, sig2_nextNonce, { from: snitching_account, gasPrice: gasPricePenalize, gasLimit: 100000000 });
			assert.fail()
		}
		catch (error) {
	...
	it("should revert an attempt to penalize relay with two transactions from different relays", async function () {
		...
		try {
			await rhub.penalize_repeated_nonce(unsignedTransaction1Encoded, sig1, unsignedTransaction2Encoded, sig2_fromAccountSix, { from: snitching_account, gasPrice: gasPricePenalize, gasLimit: 100000000 });
			assert.fail()
		}
		catch (error) {
